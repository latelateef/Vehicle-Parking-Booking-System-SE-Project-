...
if (strlen($_SESSION['vpmsaid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        $parkingnumber = time() . mt_rand(1000, 99999);
        ...
        $city = $_POST['city'];
        $quer = mysqli_query($con, "select * from tblregusers where MobileNumber='$ownercontno'");
        if (mysqli_num_rows($quer) == 0) {
            echo "<script>alert('Owner is not registered. Please register first');</script>";
            echo "<script>window.location.href ='add-vehicle.php'</script>";
            exit();
            
        }
        $user_row = mysqli_fetch_array($quer);
        $user_id = $user_row['ID'];
        ...
        $query2 = mysqli_query($con, "select id from parkinglot where city='$city'");
        $row = mysqli_fetch_array($query2);
        $parkingid = $row['id'];
        $que = mysqli_query($con, "update parkinglot set bookedSlot = bookedSlot + 1  where city='$city'");
        $query3 = mysqli_query($con, "insert into  booking(user_id,parking_id,parkinglot_id,payment_id) value('$user_id','$parkingnumber','$parkingid','Done manually')");
        if ($query3 && $query && $que) {
                                ...
                                <form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
                                    ...
                                    <div class="row form-group">
                                        <div class="col col-md-3"><label for="select"
                                                class=" form-control-label">Select</label></div>
                                        <div class="col-12 col-md-9">
                                            <select name="city" id="catename" class="form-control">
                                                <option value="0">Select City</option>
                                                <?php $query = mysqli_query($con, "select * from parkinglot");
                                                while ($row = mysqli_fetch_array($query)) {
                                                    ?>
