...
if (strlen($_SESSION['vpmsaid'] == 0)) {
  ...
  $se1 = "Select * from tblvehicle where ID='" . $_GET['viewid'] . "' ";
  $resu = mysqli_query($con, $se1);
  $row = mysqli_fetch_array($resu);
  $parkingnumber = $row['ParkingNumber'];
  $se = "UPDATE parkinglot
JOIN booking ON parkinglot.id = booking.parkinglot_id
SET parkinglot.bookedSlot = parkinglot.bookedSlot - 1
WHERE booking.parking_id = '$parkingnumber'";
  $s = "SELECT parkinglot.price
FROM parkinglot
JOIN booking ON parkinglot.id = booking.parkinglot_id
WHERE booking.parking_id = '$parkingnumber'";
  $r = mysqli_query($con, $s);
  $ro3 = mysqli_fetch_array($r);
